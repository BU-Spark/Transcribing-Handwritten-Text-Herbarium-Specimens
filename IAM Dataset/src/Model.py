...
class Model: 
	...
	def setupCNN(self):
		...
		for i in range(numLayers):
			...
			relu_drop = tf.nn.dropout(relu, 0.5)#dropout_drop_rate_ph)
			pool = tf.nn.max_pool(relu_drop, (1, poolVals[i][0], poolVals[i][1], 1), (1, strideVals[i][0], strideVals[i][1], 1), 'VALID')
	...
	def trainBatch(self, batch):
		...
		rate = 0.00001 #if self.batchesTrained < 10 else (0.001 if self.batchesTrained < 10000 else 0.0001) # decay learning rate
